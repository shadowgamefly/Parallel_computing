...
int main(int argc, char *argv[])
{
    ...
    double *coords0, *coords1, *coords2;
    double *q;
    ...
    /* Step 3 - Allocate the arrays to store the coordinate and charge
       data */
    coords0=(double *)malloc(natom*sizeof(double));
    coords1=(double *)malloc(natom*sizeof(double));
    coords2=(double *)malloc(natom*sizeof(double));

    if ( coords0==NULL | coords1 == NULL | coords2 == NULL )
    {
    ...
    for (i = 0; i<natom; ++i)
    {
        fscanf(fptr, "%lf %lf %lf %lf",&coords0[i],
               &coords1[i],&coords2[i],&q[i]);
    }
    ...
    printf("Value of system clock after coord read = %ld\n",time1);
       ...
       total_e = 0.0;             /* Total Energy (Output) */
       cut_count = 0;             /* count of included pair */
       double a_inverse = 1.0/a;  /* inverse of the constant a to reduce operation inside for loop*/
       double cut_new = cut * cut;       /*the square of threshold cut to reduce sqrt operations*/
       for (i=0; i<natom; ++i)   /*outer for loop iterate through atom*/
       {
          double i0 = coords0[i];
          double i1 = coords1[i];
          double i2 = coords2[i];
           for (j=0; j < i; ++j)  /*inner for loop iterate atom before i to avoid double counting */
           {
                   rij = (i0-coords0[j]) *  (i0-coords0[j])
                          + (i1-coords1[j]) * (i1 -coords1[j] )
                          + (i2-coords2[j]) * (i2-coords2[j]);
                   ...
                   if ( rij <= cut_new ) /* Check if this is below the cut off */
                   {
                       ...
                       total_e = total_e + current_e - a_inverse; /*add up to the final result*/
                     } /*if rij <= cutnew */
           } /* for j=1 j<=natom */
       } /* for i=1 j < i */
