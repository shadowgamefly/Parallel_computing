...
int main(int argc, char *argv[])
{
    ...
    total_e = 0.0;
    cut_count = 0;
    double c = 1/a; // constant replace function call;
    cut = cut * cut;
    for (i=1; i<=natom; ++i)
    {
        for (j=1; j<=natom; ++j)
        {
            if ( j < i )   /* Avoid double counting. */
            {
                ...
                if ( rij <= cut )
                {
                    ...
                    ++cut_count;
                    rij = sqrt(rij); //do sqrt as less as possible
                    current_e = (exp(rij*q[i-1] + rij*q[j-1]))/rij;
                    total_e = total_e + current_e - c; //turn 1-a into a constant c
                }
            } /* if (j<i) */
        } /* for j=1 j<=natom */
    } /* for i=1 i<=natom */
