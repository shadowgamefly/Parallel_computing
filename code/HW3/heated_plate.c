...
#define hotSpotRow 45
#define hotSpotCol 65
...
void printMaps(float **cells, int num_cols, int num_rows, int iterations);
...
int main(int argc, char **argv) {
	...
	for (i = 0; i < iterations; i++) {
		...
		if ((hotSpotRow < num_rows) && (hotSpotCol < num_cols)) cells[cur_cells_index][hotSpotRow][hotSpotCol]=hotSpotTemp;
	...
	printMaps(cells[final_cells], num_cols, num_rows, iterations);
...
void printMaps(float **cells, int num_cols, int num_rows, int iterations) {
	char text[255];
	sprintf(text, "Sequential_%d", iterations);
