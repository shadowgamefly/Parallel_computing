// This program simulates the flow of heat through a two-dimensional plate in parallel using MPI
...
// this simulate 10000 * 10000 plate with 10000 iterations using 2 ghost-cells within 200 tasks
...
int main(int argc, char **argv) {
	...
	for (int m = 0; m < transfer ; m++) {
		...
		if (rank % 2 == 0) {
			...
			if (rank == 0) {
				MPI_Send(cells[cur_cells_index][num_rows], ghostSize, MPI_FLOAT, 1, 0, MPI_COMM_WORLD);
				MPI_Recv(cells[cur_cells_index][num_rows + ghost], ghostSize, MPI_FLOAT, 1, 3, MPI_COMM_WORLD, NULL);
			}
			...
			// we only need odd to even upward receive and even to odd downward send
				...
				MPI_Send(cells[cur_cells_index][num_rows], ghostSize, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD);
				...
				MPI_Recv(cells[cur_cells_index][num_rows + ghost], ghostSize, MPI_FLOAT, rank + 1, 3, MPI_COMM_WORLD, NULL);
			}
		}
		else {
			...
			if (rank == nodes - 1) {
				...
				MPI_Send(cells[cur_cells_index][num_rows], ghostSize, MPI_FLOAT, rank + 1, 1, MPI_COMM_WORLD);
				MPI_Recv(cells[cur_cells_index][num_rows + ghost], ghostSize, MPI_FLOAT, rank + 1, 2, MPI_COMM_WORLD, NULL);
